...
public class ShoppingCartServiceImpl implements ShoppingCartService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void addToCartBatch(List<OrderDetail> orderDetails) {
        Long userId = BaseContext.getCurrentId();
        ...
        for (OrderDetail detail : orderDetails) {
            ...
            ShoppingCart.ShoppingCartBuilder builder = ShoppingCart.builder()
                    .userId(userId)
                    .number(detail.getNumber() != null ? detail.getNumber() : 1);
            ...
            if (hasDish) {
                ...
                if (dish.getStatus() == null || dish.getStatus() != 1) {
                    throw new BaseException(MessageConstant.DISH_IS_NOT_ON_SALE);
                }
                builder.name(dish.getName())
                        .dishId(dishId)
                        .dishFlavor(detail.getDishFlavor() != null ? detail.getDishFlavor() : "")
                        .amount(dish.getPrice())
                        .image(dish.getImage());
            } else {
                Setmeal setmeal = setmealMapper.getById(setmealId);
                ...
                if (setmeal.getStatus() == null || setmeal.getStatus() != 1) {
                    throw new BaseException(MessageConstant.SETMEAL_IS_NOT_ON_SALE);
                }
                builder.name(setmeal.getName())
                        .setmealId(setmealId)
                        .amount(setmeal.getPrice())
